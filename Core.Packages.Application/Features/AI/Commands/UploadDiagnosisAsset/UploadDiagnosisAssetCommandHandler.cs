...
public sealed class UploadDiagnosisAssetCommandHandler : IRequestHandler<UploadDiagnosisAssetCommand, IDataResult<UploadDiagnosisAssetResponse>>
{
    ...
    private readonly IEntityRepository<MediaAsset, int> _assets; private readonly IFileStorageService _storage; private readonly ITenantService _tenants; private readonly IHttpContextAccessor _http;
    public UploadDiagnosisAssetCommandHandler(IEntityRepository<MediaAsset, int> assets, IFileStorageService storage, ITenantService tenants, IHttpContextAccessor http) => (_assets,_storage,_tenants,_http)=(assets,storage,tenants,http);
    public async Task<IDataResult<UploadDiagnosisAssetResponse>> Handle(UploadDiagnosisAssetCommand request, CancellationToken ct)
    {
        ...
        var clientId = _tenants.GetRequiredClientId(); var key = await _storage.UploadFileAsync(request.File, $"ai-drafts/{clientId}/{userId}", ct);
