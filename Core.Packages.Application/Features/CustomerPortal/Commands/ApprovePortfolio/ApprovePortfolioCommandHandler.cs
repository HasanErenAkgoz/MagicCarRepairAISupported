...
{
    public class ApprovePortfolioCommandHandler : IRequestHandler<ApprovePortfolioCommand, IResult>
    {
        ...
        public ApprovePortfolioCommandHandler(
        ...
        public async Task<IResult> Handle(ApprovePortfolioCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return new ErrorResult("UNAUTHORIZED_PORTFOLIO_APPROVAL");
            }

            // Onay durumunu güncelle
            portfolio.CustomerApprovalStatus = CustomerApprovalStatus.Approved;
            portfolio.CustomerApprovalDate = DateTime.UtcNow;
            portfolio.CustomerRejectionReason = null;

            // Eğer yayınlanacaksa, yayın tarihini ayarla
            if (portfolio.IsPublished && !portfolio.PublishedDate.HasValue)
            {
            ...
            await _signalRNotificationService.SendNotificationToClientAsync(
                clientId,
                "Portföy Onayı",
                $"Müşteri #{currentCustomer.Id}, iş emri #{portfolio.WorkOrder.WorkOrderNumber} için portföy onayını verdi.",
                "Portfolio",
                portfolio.Id);

            return new SuccessResult("Portfolio approved successfully");
        }
    }
}
